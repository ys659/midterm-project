...
def test_integer_division():
    ...
    assert calc.result == Decimal("-3")  # truncates toward zero
...
def test_percentage_by_zero():
    with pytest.raises(OperationError, match="Division by zero is not allowed for percentage"):
        Calculation(operation="Percentage", operand1=Decimal("5"), operand2=Decimal("0"))

def test_absolute_difference():
    calc = Calculation(operation="AbsoluteDifference", operand1=Decimal("5"), operand2=Decimal("9"))
    assert calc.result == Decimal("4")
